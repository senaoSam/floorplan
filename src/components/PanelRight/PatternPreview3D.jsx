...
//   vertical drag — tilts the preview CAMERA pitch (view-only, not persisted).
//     Azimuth is a horizontal quantity, so the vertical axis is free for
//     inspecting the lobe from above / the side / below. (A real antenna
//     tilt parameter is Phase 40 — this axis will switch to editing it.)
//
...
function drawPreview(canvas, pattern, color, azimuthDeg, pitchDeg) {
  ...
  // Boresight direction on the ground plane (dashed, like the 2D preview).
  ctx.beginPath()
  ctx.moveTo(cx, cy)
  ctx.lineTo(cx + MAX_R * Math.cos(azRad), cy + MAX_R * Math.sin(azRad) * sinP)
...
export default function PatternPreview3D({ pattern, color, azimuth = 0, onAzimuthChange }) {
  const canvasRef = useRef(null)
  // Local azimuth while dragging (unwrapped accumulation for a smooth spin);
  // null = follow the store azimuth.
  ...
  const dragRef = useRef(null)   // { startX, startY, startAz, startPitch, lastSent }
  const displayAz = dragAz ?? azimuth

  useEffect(() => {
    if (canvasRef.current) drawPreview(canvasRef.current, pattern, color, displayAz, pitchDeg)
  }, [pattern, color, displayAz, pitchDeg])
  ...
  const onPointerMove = (e) => {
    ...
    if (!drag) return
    // Horizontal → azimuth. Preview-local only while the button is held —
    // the store (and thus any heatmap recompute) is untouched until release.
    const nextAz = Math.round(drag.startAz + (e.clientX - drag.startX) * AZ_DEG_PER_PX)
    if (nextAz !== drag.startAz) drag.moved = true
    setDragAz(nextAz)
    // Vertical → view pitch (drag down = look from above, up = from below,
    // matching OrbitControls' orbit direction in the 3D viewer).
  ...
  const onPointerUp = () => {
    ...
    if (drag.moved && dragAz != null && onAzimuthChange) onAzimuthChange(wrapAzimuth(dragAz))
    setDragAz(null)
  }
