...
| HM-F5c-fix-2 | ⬜ | **Kahan / double-float for destructive nulls**：解 basic (10, 13) 那種 「power=0 cancellation 點」fp32 累加極限。每個 H accumulator 加 compensation term，coherent sum 改 Kahan summation。成本約 +30% shader ALU 但只發生在 reflection path（refl=on 才會多 path 干涉），direct path 不付。**驗收**：basic full physics max ≤ 1 dB（目前剩 1/2501 cell @ 32 dB）|
