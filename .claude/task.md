...
| HM-drag-lod | ✅ | **拖曳期間降畫質**：HeatmapLayer 從 useDragOverlayStore 衍生 isDragging（任一 dragAP/dragWall/dragScope 非 null 即為 true）；拖曳期間 gridStep × 3、freqOverrideN=3 餵給兩條 sample engine 路徑。**位置/牆/AP 形狀全正確**，只是邊緣糊 + 反射區數值抖 ±5 dB；放手 overlay 清空，下一輪 effect 自然回到完整精度（~35 ms）。預估加速：reflection-off ≈ 9× / reflection-on ≈ 45×（樣本 × 1/9 + 多頻點 15→3）|
| HM-F5i | ⬜   | **Refl/Diff 接上 wall grid（shader-only 路線主軸 A）**：目前 `accumulateWallLossExcept`（反射用）和繞射段的 `accumulateWallLossWithHits` 都註明「grid duplicates wall references, complicates skip semantics」走 brute-force `for w in 0..uWallCount`，沒用既有的 `uGridIdx/uGridList` DDA。把這兩條路徑改用 grid 走格 — skip semantics 解法：DDA 命中的 wall index 與 `excludeWallIdx` 比對跳過、用既有 SEEN_BUF=16 cyclic dedup 防重複。**預期效益**：refl/diff 的 per-fragment wall scan 從 O(N_walls) 降到 O(√N_walls) 量級，25 牆約 4×、100 牆約 10×。**驗收**：refl-min + basic + dense-aps + dense-walls + cross-floor-tunneling 全部 F5d gate ≤ 1 dB（接受 basic (10,13) 1-cell known issue），bench 場景對 F5g 量測加速比 |
| HM-F5j | ⬜   | **Per-AP LOS field bake（shader-only 路線主軸 B-1，drag 妥協回收）**：每個 AP 預算一張 R8 grid（或 packed bitmask），記「從這個 AP 到此 cell 是否 LOS」。fragment shader 在 main() 開頭先查這個 bit — LOS=1 走純 Friis 跳過所有 wall scan/反射/繞射；LOS=0 走完整 refl/diff。記憶體：25 AP × 256×256 × 1 byte ≈ 1.6 MB。**Invalidation**：AP 移動只 invalidate 該 AP 那一行；牆編輯 invalidate 整張。**最大效益**：把 d29027e 那個「拖曳期間強制 maxReflOrder=0 + enableDiffraction=false」的妥協拿回來 — LOS field 涵蓋 80%+ cell，剩餘 20% 走完整 refl/diff 在拖曳預算內。**驗收**：拖曳期間保留完整 refl/diff，仍維持 60 FPS；fixture 對齊不變 |
| HM-F5k | ⬜   | **AP→corner / AP→wall 鏡像 precompute texture（shader-only 路線主軸 B-2/B-3）**：bake 兩張 per-AP texture：(a) AP→corner 的 (距離, AP→corner LOS bool, AP 端 Fresnel v 的一半)，N_AP × N_corners × RGBA32F；(b) AP→wall 鏡像點 (x, y)，N_AP × N_walls × RG32F。fragment shader 反射/繞射段不再 per-fragment 重算 mirrorPoint() / segSegHit() 的 AP 端那一半，繞射 wall scan 工作量砍半。AP 不動就持續有效，配合 HM-F5j 同一套 invalidation。**驗收**：refl-min + basic full-physics gate ≤ 1 dB；refl/diff 大場景對 HM-F5i 量測加速比 |
| HM-F5l | ⬜   | **Coarse-fine 擴張到 refl/diff（shader-only 路線主軸 C）**：HM-F5h 的 coarse pass 目前只判斷「free-space 是否 alive」，擴張成同時輸出 `needReflDiffMask` channel — coarse cell 距最近牆 > threshold 或 free-space 已被擋的 cell 標 false。fine pass 看到 mask=false → 走 Friis only 早早 return；mask=true → 走完整 refl/diff。可整合到 HM-F5j 的 LOS field 同一張 texture 的另一個 channel。**驗收**：cross-floor + dense-walls full-gate 不 regress；大場景 refl/diff 比例下降比例與實測加速比 |
| HM-F5m | ⬜   | **F5c+ 改 4-AP batch（解 readPixels roundtrip）**：refl/diff 場景目前走 per-AP renderAp + N_AP 次 readPixels（sampleFieldGL.js per-AP fallback path）。改成單 pass 處理 4 AP 寫到 RGBA32F 的 4 個 channel，N_AP/4 次 pass + N_AP/4 次 readPixels。register 壓力評估：4 AP × NMAX=64 相干和陣列若爆，降到 2 AP batch（RG32F）。**驗收**：refl-min + basic full-gate 不 regress；100 AP refl-on 場景對 HM-F5g per-AP fallback 量測加速比（預期 3-6×）|
