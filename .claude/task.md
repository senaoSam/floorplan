...
| 33-13 | ✅   | **ClientPanel UI 統一**：改用共用 `_shared/PanelControls`（Select / NumberInput / Checkbox），跟 AP 面板同款（發射功率 `[N] dBm` 同一元件）；移除自製 select/slider/number；`--panel-accent: #4fc3f7` 配色對齊。MCP 並排確認與 AP 面板一致 |
| 33-14 | ✅   | **手動鎖定 AP（右鍵選單）**。設計師 what-if：右鍵 AP→專屬輕量選單（`ClientViewMenu`，復用 tray-ctx-menu 樣式，非一般物件選單）「手動連接此 AP」；右鍵其他/已鎖定→「解除手動連接」。store `lockedApId`。**右鍵走原生 `contextmenu` 事件**（PIXI v8 federated 不發右鍵 pointerdown，binder 掛 canvas contextmenu listener + AP hit-test）。`simulate.pickServing` 鎖定 AP 在 candidates 就強制 serving；**否則回 `{serving:null,lockUnreachable}`——不 fall back 連別台**（尊重使用者選擇，連不到就明確報、不畫線、保留鎖定）。**藍色 association 維持全網不變**。面板顯 🔒 badge + 解除鈕 + lockUnreachable 警示。MCP 驗證 lock/unlock/serving 切換、右鍵 hit-test、遠處鎖定無線+警示 |
| 33-15 | ✅   | **手動鎖定互動細修**：①右鍵連不到的 AP（用 client 當前位置算 RSSI<-85 或未放置）→選單「手動連接」直接 disabled+提示（訊號太弱／請先放置 client）；reachability 在 binder 右鍵時用 buildCandidates 預算、塞進 cvMenu。②選單開啟時左鍵/右鍵點畫布別處＝**只關選單、不移動 client**（要選單關閉後再點才移動）；解 race：menu 的 outside-click 跳過 canvas 點擊交給 binder。MCP 驗證：menu 開→左鍵遠處→只關 pos 不變→再點才移動；AP-02 可達 enabled / AP-05 不可達 disabled+hint |
| 33-16 | ✅   | **單台 AP 涵蓋（single-AP coverage）**。全域藍網維持不變，額外標示單一 AP 的良好訊號範圍。①serving 自動：client 連的那台預設顯示。②右鍵 AP→選單「顯示/隱藏 此 AP 範圍」。**手動優先**（`outlineApId = singleApAreaId ?? servingApId`），手動選固定不被 serving 覆蓋、隱藏後回 serving。**同時只一個**。與 lockedApId 獨立。`association.computeSingleApArea`（同 coverage sweep 限定該 AP RSSI≥門檻）；store `singleApAreaId`+`singleApArea`。**視覺（使用者迭代定案）**：紅色 #ef4444 ——整塊**紅色半透明填充 alpha 0.3 + 紅邊**（疊全域藍上，明顯標出哪台）+ 該 AP marker **內部填紅**（apsLayer drawAP，非外圈）。MCP 驗證 serving/手動/隱藏切換 |
| 33-17 | ✅   | **CLIENT_VIEW 專屬 hover 回饋**。read-only 模式 cap 本關掉 hover→使用者不知 AP 可右鍵操作。apsLayer `pointerover` 加 CLIENT_VIEW 例外（cursor 維持小人不變 grab）。drawAP 加 `isCvHover` 分支：hover 一般 AP→**白底+紅粗外框+紅字**（預覽感、暗示右鍵看紅色範圍）；hover 已顯示範圍的 AP→維持紅底。訂閱 useClientViewStore 讓 highlight 變化重畫。MCP 驗證：CLIENT_VIEW hover→hover store 設定+紅框、移開清除 |
