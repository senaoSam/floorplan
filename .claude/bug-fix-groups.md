...
2. **G1 必須最早** — 其中一條白屏會阻斷 4 點校正功能的所有測試,QA 兩輪都被卡住

---

# G1 · 白屏與功能阻斷(5-6 小時)★ 最優先

修法明確、都有正確範例可對照、風險低。做完能解除 QA 的測試阻斷。

| # | 位置 | 問題 | 修法 |
|---|---|---|---|
| C1 | `CameraTimeline/CalibrationModal.jsx:109` vs `:179` | `useOverlayDismiss` 在 early return 之後 → 點「校正熱圖」**必定白屏** | 把 `:179` 的 hook 移到 `:109` 之前。對照 `LiveViewModal.jsx:59` 的正確寫法 |
| C2 | `viewer3d/WallLayer3D.jsx:140-153` | 零長度牆 → hook 數量變化 → 白屏。端點吸附會產生完全相等座標 | early return 移到所有 hook 之後 |
| C2b | `viewer3d/TrayLayer3D.jsx:24` | 同上,退化 tray 線段 | 同上 |
| — | 全庫 | 確認沒有第 4 處 | 已用腳本掃過(10 個候選、7 個誤判),**只有這 3 處**,無需重掃 |

**驗收**:點「🎯 校正熱圖」能正常開啟 modal;拖曳牆端點到與另一端重合不崩潰;3D 模式下同樣操作不崩潰。

**做完後追加**:C1 修好後,**4 點對位校正的完整流程從未被真人驗證過**(QA 兩輪都被白屏阻斷)。請在此組結束時走一次完整校正流程,順便驗 T13(`CalibrationModal.jsx:82` 的 effect 依賴只有 `[calibrateCameraId]`,切樓層來回會混用不同 letterbox 換算的四點)。
...
若想更早看到成效,前三組(G1 + G2 + G5,約 19-24 小時)覆蓋了「會白屏」「讓驗證失效」「改錯無法回退」三類最傷的問題,做完系統的可信度會明顯提升。
