...
| **反射 `maxReflOrder:1`** | **10.07 dB / 197 格** | ❌ **新發現,未在報告中** |
| **反射 + 繞射** | **6.97 dB / 153 格** | ❌ 同上 |
| **同樓板兩個洞** | **61.24 dB / 44 格** | ❌ = 既有的 **P1-7** |

**兩點結論**:
1. **P1-7 得到實測數字**:GL 完全忽略第二個洞,JS 有算。單洞時兩者 0.000 dB 完全一致 → 問題確定只在「第二個以後的洞」。
2. **反射路徑是新發現的分歧**(~10 dB),原報告 93 條裡沒有。建議併入 G3 一起處理,因為同樣需要對照 JS 參考實作。

**⚠ 比對時的陷阱(不知道會誤判)**:兩個引擎對「無訊號」格的表示法不同 —— JS 寫 `-300`,GL 寫巨大負值(實測 `-453927`)。直接相減會得到 **677651 dB** 這種荒謬數字,看起來像大 bug 其實是 sentinel 差異。**排除 `< -200 dBm` 的格之後,5GHz/6GHz 的真實差異是 0.000 dB。** G3 寫 harness 時務必先過濾。
