...
| 16-3b | ⏸️ 暫緩 | 圖框與外邊界偵測：findContours 找最大封閉輪廓 → 內縮 N px 當分析 ROI，後續所有處理只在 ROI 內，順便解掉「上傳圖有多餘白邊」。**2026-04-30 評估後暫緩**：原規格對「沒有圖框、最大輪廓就是建築外牆本身」的圖（如 test-floorplan.png）會把外牆 4 條剃掉且不補回；實作試過一次已 revert。實際想剔除的 scale bar / 標題 / 圖例都不是封閉輪廓，findContours 抓不到，得靠 16-3d (component scoring) / 16-3i (parallel pair) / 16-3l (confidence scoring) 處理。若未來重做需先加「貼邊判斷」（最大 contour 必須四邊都離影像邊緣 < N px 才視為頁框），且預設關閉，使用者明確標示「我這張圖有頁框」時才開 |
| 16-3c | ✅   | Deskew：Hough 統計線段角度直方圖估主方向 → warpAffine 旋轉校正。**必須在角度量化前做**，否則掃描圖歪 2-3° 整片誤判。**已實作**（2026-04-30）：worker `estimateSkewAngleDeg` 用 HoughLinesP（minLineLength=diag×3%、maxLineGap=4、threshold=80）取線段，每條角度 fold 到 [-45°, +45°] (mod 90°，水平 + 垂直牆共票)，長度加權 0.5° bin 直方圖，peak ±1 bin 重心化做 sub-bin 細化。`deskewMat` 用 `getRotationMatrix2D` + `warpAffine` (INTER_LINEAR + BORDER_CONSTANT 0)，warp 後 `threshold(127)` 重新二值化消除插值灰邊。Skip 條件：`!deskewEnabled` / `samples < minSamples` (預設 30) / `|angle| < minDeg` (預設 0.3°，已對齊) / `|angle| > maxDeg` (預設 15°，疑似誤判旋轉建築不是 skew)。Debug page 加 `Deskew` checkbox + min/max° 輸入 + stats 顯示 applied 角度 / samples / peak weight 或 skipped reason。test-floorplan.png 跑出 `skipped (within-min-deg) · estimated ~0°` 視覺無變化（no-op 驗證通過） |
...
| 16-3e | ✅   | 分方向 morphology 強化：horizontal kernel `Size(15,1)` + vertical kernel `Size(1,15)`（kernel 長度依 deskew 後主方向 + 估計牆厚動態調），對角 kernel optional。把水平、垂直牆線連起來，斷裂的虛線變實線。**目前實作改用「long-line mask」做法**（不是直接 OPEN binary）：(1) `lineMask = OPEN(H, K) ∪ OPEN(V, K)` 偵測長線位置 (2) `keepMask = dilate(lineMask, dilatePx)` 加容差 (3) `result = src AND keepMask` ——保留**原圖細節與線厚**，只擋掉「附近沒有長線支撐」的孤立區塊（文字/家具）。**踩坑**：先前直接 OPEN 會把細牆 erode 沒、把厚薄不一的牆切成半紅半白；改成 mask 路線後牆厚不被破壞，文字仍可消除。Debug page 提供 Morph K + Mask dilate 兩個參數。預設 K=15、dilate=3，待 16-3i 估牆厚後改自適應。待使用者驗證後標 ✅ |
| 16-3f | ✅   | HoughLinesP 線段抽取：minLineLength = imageDiagonal × 0.01~0.03（相對尺度，不要寫死 px）；maxLineGap = estimatedWallThickness × 1.5~3。先估牆厚再決定參數。**已實作 baseline**：worker 內 binarize 之後接 HoughLinesP，吐出 segments + segStats（count / totalLengthPx）；debug page 三欄顯示（原圖 / binary / binary+紅色線段）並提供 houghThreshold / minLineLengthRatio / maxLineGapPx 三個參數即時調整；**目前 maxLineGap 仍是固定 px**，待 16-3i 估牆厚後改成自適應。待使用者驗證 baseline 後標 ✅ |
...
| 16-3h | ✅   | Graph-based collinear merge：node = candidate segment，edge = `angleDiff < tol && offsetDiff < tol && projGap < tol`（用方向向量 u 與法向量 n 的投影統一處理任意角度），connected components 後對每個 component 投影最小/最大端點為合併線段。比兩兩比對乾淨且無順序問題。**已實作** `src/utils/aiWalls/mergeSegments.js`：規範化方向（avoid 反向重複）、用中點計算 offset 較穩定、長度加權平均 u 為 component 共識方向、投影 min/max 取端點。Union-Find O(n²) 連通；HoughLinesP 輸出量級 100~2000 不需 spatial index。Worker inline 同份邏輯（classic worker 不能 import）。**+ Endpoint extension（搭配此 stage）**：合併後對每條線段兩端沿 u/-u 方向用 morph mask 探測延伸（`extendMissThreshold` 連續空白 px 才停），補回 HoughLinesP 在端點 vote 不足砍掉的最後幾 px。Debug page Overlay 可切換 raw(紅)/merged(綠)/both，並提供 angleTolDeg / offsetTol / gapTol / extendMiss 四參數。預設 5° / 4px / 12px / 3。待使用者驗證後標 ✅ |
| 16-3i | ✅   | **Wall thickness 估計 + adaptive morph（parallel pair detection，2026-04-30）**：`src/utils/aiWalls/estimateWallThickness.js`（worker 內 inline 同份邏輯）。對 16-3h 合併後的每條 segment：找方向夾角 ≤ `pairAngleTolDeg`、法向距離 ∈ [`minThicknessPx`, `maxThicknessPx`]、沿 u 方向投影重疊 / min(len_i, len_j) ≥ `overlapRatio`、且距離 ≤ `relMaxRatio × min(len)` 的最近平行 segment 配對（O(n²)，n 是 post-merge ~50-500 不需 spatial index）。`relMaxRatio=0.3` heuristic：牆長 >> 牆厚，超過自身長度 30% 的法向距離幾乎不可能是「同一面牆兩條邊」（殺穿越中庭的長對角線假配對）。配對距離取 **median** 當 `estimatedPx`（robust against scale-bar / 重複圖樣 outlier；length-weighted peak 同步保留 `peakPx` 當診斷信號，median 跟 peak 數字差很大時代表有 length 偏置）。預設 pairAngle=5°、min=2、max=30、overlap=0.7、relMax=0.3。**前一版**用 length-weighted histogram peak，但 test-floorplan.png 上方 scale bar 兩條黑/白色塊相距 ~22 px 又長又乾淨，length 總和壓過真正的細牆，把 peak 帶到 22 px（真實牆厚 ~4 px）。改用 median 後同 fixture estimatedPx ~4 px、peakPx ~22 px，median 不被 outlier 拉走、peakPx 變診斷信號可看出有無 outlier。**Adaptive morph（同 commit）**：把 morph→hough→merge→extend→thickness 抽成可重入 `runMorphAndHough(K, maxGap)`；pass 1 跑使用者設定的 K，若 estimatedPx 可信（**三個 guard 都通過**：≥ 5 paired + 配對率 ≥ 30% + estimatedPx ≤ 50 px）且 `target K = clamp(round(3·est), [3, max(initialK, 19)])` 與 initialK 差 ≥ 4 px → pass 2 用 (target K, target maxLineGap = round(2·est)) 重跑。**配對率 30% 門檻（從 15% 提高）**：單線繪製的圖（test-floorplan.png 那種）配對都是「兩面相對的牆」而非「同面牆兩邊」，配對率通常 15-25%；雙線繪製圖通常 50%+。30% 是經驗分界，避免單線圖把 estimate=22 px（兩面對牆距離）當牆厚回灌觸發 pass 2 把 K 推大。**est ≤ 50 px 上限**：牆厚物理上不可能 > 50 px，超過代表配的是房間寬度。**K 上限保護**：用 `max(initialK, 19)` 而非寫死 25，因為 K 估錯時往「變大」方向風險高（會把細牆 erode 沒），「變小」方向風險低（最多 morph mask 不收斂、輸出回退到 raw binary），所以只允許 K 大幅縮小、不允許大幅長大。Debug page 加 5 個參數（pairAngle/min/max/overlap/relMax）+ adaptive checkbox（預設 on）+ stats 行「Wall thickness ~X px · paired N/M」+「Adaptive: pass-1 K=A → pass-2 K=B · gap A→B · trigger est ~X」+ overlay `paired` 模式（成對綠 / 未成對暗灰）。輸出 `wallThickness: { estimatedPx, pairedCount, totalCount, histogram, perSegment[].paired/pairedDist/pairedIdx } + adaptive + morphKernelUsed + maxLineGapUsed`。Smoke test：4 條相距 4 px 的平行牆 → estimatedPx=4 全配對；50 px 距尺寸線 + 孤立家具邊 → unpaired，邏輯正確。待使用者瀏覽器驗證 4 張測試圖後標 ✅ |
...
| 16-3l | ✅   | **候選 confidence scoring 整合（minimal viable，2026-04-30）**：`src/utils/aiWalls/scoreSegments.js`（worker 內 inline 同邏輯）。三 feature 加權：`score = 0.30·lengthScore + 0.45·pairedScore + 0.25·densityScore`，全部子分數先 normalize 到 [0, 1] 再加權（同 HM-F4 cost-fn v2 lesson）。**(a) lengthScore**：`len / diag` clamp 到 0.05 對角線封頂（一條 5% 對角線的牆已經達 1.0，再長無加分）。**(b) pairedScore**：16-3i `perSegment[i].paired` ? 1.0 : 0.3（floor=0.3 避免單線繪製圖被一刀殺，paired 仍是強訊號）。**(c) densityScore**：worker 內 `sampleSegmentDensity` 在 morph mask 上做 oriented ROI 掃描——沿 segment 方向每 px 一步，垂直方向採 2*halfW+1 個 transverse sample（halfW 從 16-3i estimatedPx 動態決定，預設 3）；hits/total 線性映射 [0.4, 0.9] → [0, 1]，< 0.4 視為 noise（家具 / 尺寸線斷裂）、≥ 0.9 為 max。Bucket 門檻：score ≥ 0.65 = high、≥ 0.35 = medium、其餘 = low（high 自動寫入、medium UI 確認、low 隱藏，給 16-3n review UI 用）。Debug page 加 `Confidence buckets (16-3l)` overlay 模式（low 灰 / medium 黃 / high 綠）+ stats 行 `high N (X%) · medium N · low N`。**先實作核心三 feature，未完成的 topology / angle-cluster / symbol-penalty / dimension-line-penalty 等留 16-3d/g/j 補進來時再加 weighted term，API shape 不變**。權重 preset（CAD / 掃描 / 手繪）也留待實際使用 review UI 後校調 |
...
| 16-3o | ✅   | **整合入口（minimal viable，2026-04-30）**：Toolbar 右側加「🤖 AI 牆」按鈕（active floor 沒 imageUrl 時 disabled），點擊開 `AIWallsModal`（Portal 渲染）。流程：載入 floor.imageUrl → ImageData → post 給 classic worker → 顯示 segments 數 + 三桶統計 + 預覽縮圖（綠/黃/灰）→ 三按鈕「寫入高信心 / 寫入高+中 / 寫入全部」。寫入時 worker 回傳的 `mergedSegments` 已經是 image-pixel 座標，直接當 `{startX, startY, endX, endY}` 用（floor image 的 canvas 座標 = image pixel 座標一致）。新牆 append 到既有 walls（不取代手繪）；history store 自動 subscribe wallStore.setWalls → Ctrl+Z 整批回退。**16-3n review UI 暫不實作**：modal 用「三按鈕分桶寫入」當代用，使用者覺得 medium/low 太雜可以只寫 high；要刪個別誤判就用主編輯器既有的選取/刪除。等實際用過再決定 review UI 要不要做、做成什麼樣。**已知 issue（2026-04-30）**：scale bar / 頁框 / 標題文字會被當牆寫入（test-floorplan.png 為例）。短期解法：使用者先用裁切工具框出真實平面圖範圍再跑 AI；長期解法等 16-3b（圖框 ROI 偵測）+ 16-3l 補 textCluster/borderPenalty/dimensionLinePenalty term 一起處理。當前 minimal viable 不擋此 case |
