...
| HM-F5l | ⬜   | **Coarse-fine 擴張到 refl/diff（shader-only 路線主軸 C，物理保真）**：HM-F5h 的 coarse pass 目前只判斷「free-space 是否 alive」，擴張成同時輸出 `needReflDiffMask` channel。**判據必須走物理 upper bound（不是 heuristic）**：coarse cell 的 free-space-only RSSI 已 < cullFloor → 此 cell refl/diff 也不可能救回（refl/diff 都是衰減，物理上限 = free-space），mask = false 安全；fine pass 看到 mask = false → 走 Friis only 早早 return（與 HM-F5h 同一個零誤差論證）。**禁止**用「距最近牆 > threshold」這類 heuristic — 反射可從遠牆射過來。可整合到 HM-F5j 的 LOS field 同一張 texture 的另一個 channel。**驗收**：refl-min + basic + dense-aps + dense-walls + cross-floor-tunneling 全部 full-gate ≤ 1 dB 不 regress（數學上零誤差）；大場景 refl/diff 比例下降比例與實測加速比 |
