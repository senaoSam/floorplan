...
**牆體厚度（純視覺）完成（2026-09-11，使用者驗收 ok）：牆加 `thicknessM`（預設 0.1m），2D 畫成實心帶 `max(真實厚度, 原3px)`、3D 取代寫死的 0.1、命中範圍 `max(帶半寬, 14螢幕px)`、WallPanel + 新增畫布左上「畫牆預設」面板（材質色塊格 + 厚度，單位 cm 整數）、移除 Tab 切材質。厚度只驅動視覺，不進 RF 引擎（牆對引擎仍是零寬線段，衰減是 per-crossing 不是 per-metre）。**

> **轉角補角：實作完成後依使用者決定整包撤回（2026-09-11）。** 厚牆轉角的外角（邊長＝半個牆厚）視覺上被 round cap 遮住、但幾何上不在任何線段上，射線穿過零衰減——實測 10cm 牆洞 4.4cm／30cm 牆 13.1cm／70cm 牆 32.8cm。曾完成 L/T/十字 miter 楔子（各半材質色、`MITER_LIMIT=4`、只補凸側），驗證通過後使用者決定「恢復圓角」，程式碼已移除。
> **防重做 — 兩個結論**：①`cap: 'butt'` 不能替代楔子：實測它讓外角凸出、內角凹陷成階梯，比圓頭更醜且洞仍在原處。②主流牆厚 10–15cm 產生的洞（5–7.5cm）小於或約等於 2.4GHz 波長（12.5cm），物理上繞射截止，修它不會讓熱圖更準；洞開始有意義的門檻在 25cm 以上（RC 外牆/剪力牆/電梯井）。若要修，最小改動是 `buildScenario` 牆端各延伸半厚度（JS/GLSL 共用同一份 scenario，parity 風險低），但會改變轉角附近熱圖數值、須重驗。
> **待決**：厚度上限目前 200cm（隨手設、無依據），建議降到 40cm（涵蓋剪力牆/電梯井，排除洞失控的區間）。

**★ 2026-08-19：Phase 53 全部完成（G1~G10，使用者驗收 ok）。** 三輪 bug 獵捕找出 93 條
（含 4 條 critical，其中 2 條會白屏），已分成 10 組 × 5-10h。**G1~G10 ✅ 全部完成，4 條 critical 全修完。R1「反射分歧」查證為 harness 誤判、非 bug。
...
**55-F 使用者回報兩個小問題修復（2026-09-22）**

**① 切樓層後拖曳鏡頭會被拉回（「有時會有時不會」）**
根因：OrbitControls 的 `start` 事件只停 auto-rotate（`stopSpin`），**從不取消進行中的 tween**。切樓層的升降是 `k=8` 漸近 lerp，要收斂到 `1e-4` 才關掉 `tweening`；在那幾百毫秒內抓鏡頭，等於兩邊每幀都在寫 `camera.position`——orbit 移走、lerp 拉回。「有時」就是看你拖的時候 tween 收斂完了沒。
**實測還發現更糟的一面**：如果一直拖到 tween 的收斂判定之後，**升降會無聲消失**——樓層切到 5F 了，鏡頭還停在舊樓層高度（camY 25.89 永遠到不了 37.89）。同一個根因的兩張臉。
**修法**：`start` handler 改名 `yieldToUser`，除了停轉盤，還 `tweening=false` + 把 desired 目標吸附到使用者當下的位置（讓目標無法再反撲）。
**連帶必須做的**：`lastTarget` 一併重設為 `[NaN,NaN,NaN]`。因為它是在 tween **開始**時就記錄，被取消的那次會被誤記成「已經到過」，導致**之後每次切回該樓層都比對相等而跳過升降**（實測確實整個 session 再也不升降）。NaN 不等於任何值，下次必定重算。

**② CAMERA 模式沒有熱圖，右上角卻還有「各層熱圖」鈕**
`layerVisibilityBinder.js:44` 已經讓 heatmap layer 在 camera 模式 `visible = false`，所以那顆鈕在控制一個畫不出來的東西。改為 `!inCameraMode &&` 條件渲染。
實測佐證：camera 模式 `heatLayerVisible: false` 且鈕消失；切回 plan 兩者同時回來。

**②-b 兩顆鈕的左右順序對調（使用者回報）**：條件渲染做完後，**「自動旋轉」會在切 camera 時橫移 84px**
（實測 left 1735 → 1819）。因為該列是 `justify-content: flex-end` 靠右對齊，**被移除的若是右邊那顆，左邊的就會滑過去補位**。
改成「各層熱圖」在左、「自動旋轉」在右 —— 會消失的那顆排在前面，模式切換時留下來的東西就都不動。
**量化驗證**：`自動旋轉` 在 plan / camera / 切回來三種狀態 left 恆為 **1819**；三個相機預設位、樓板三段式 track、面板寬度切模式前後全部一致。
> **原則**：右對齊的列裡，**可能被條件渲染掉的元素要排在前面**，否則它的消失會推動其他控制項。

**驗證（0 console errors）**：
- 正常升降 5F=37.89 / 3F=31.89 / 1F=25.89
- 拖曳凍結 → 放開升到 37.89（決策 5 未回歸）
- **抓 tween 中的鏡頭 → 姿態完全保留**（使用者拖到哪就停在哪，實測 camY 33.27 這種中間值）
- **取消後的復原**：再切 1F=25.89 / 5F=37.89 正常升降（`lastTarget` 重設生效）
- 相機預設位、2D→3D 進場動畫皆無回歸

> **量測陷阱（第二次踩到，記起來）**：電梯條吃的是 **pointerdown**，用 `element.click()` 這種合成事件**完全不會切樓層**——
> `activeFloorId` 紋風不動。我一度因此誤判「修法沒生效」，實際上是測試腳本沒觸發到功能。
> **驗電梯條一律用 `page.mouse.move/down/up`，不要用 `.click()`。**

### Phase 56 3D 樓層標籤塔（Floor Tab Tower）— **2026-09-22 完成**

**定位**：3D 空間中貼著樓板邊緣的可點樓層牌。解決「視線不離開模型」的空間對應——看著熱圖/線路時，要切的樓層就在那層旁邊，不必移到畫面邊緣的電梯條。
與 Phase 55 電梯條**互補而非取代**：電梯條保證「所有樓層都列得出來」，標籤塔負責「看著模型順手點」。

**新檔**：`FloorTabTower3D.jsx`、`tabPointerGuard.js`
**改檔**：`Label3D.jsx`（加 `interactive` / `variant` 兩個 prop）、`Viewer3D.jsx`（掛載 + floorSizes）

**最終設計（都是實測逼出來的，不是一開始想到的）**

| 決策 | 為什麼 |
|---|---|
| **對角兩面**（長邊那兩側） | 先做單面 → 相機繞圈會被擋，加了「依相機選邊」→ **牌子有時在左有時在右、使用者無從預期**（使用者回報）→ 改四面 → 同層同時出現 3~4 塊太吵（使用者回報）→ **對角兩面**：互相背對，任何角度必有一面在近側，且不需要任何切換/淡入淡出邏輯 |
| **固定螢幕大小**（非透視縮放） | 透視縮放時 3m 樓高在螢幕上只有 ~48px、hit 區差不多大小 → 相鄰樓層互相干擾、點錯層；拉遠則小到讀不出來。改成每幀依距離反算縮放，pill 與 hit plane 用同一個 `k`（只縮 sprite 的話點擊區會跟著縮小，那才是要緊的一半） |
| **貼著樓板邊緣**（`TAB_EDGE_GAP_M = 0.55`） | 原本站在建築角落外側 → 透視把它縮到最小、又跟右側電梯條重疊。貼邊後歸屬一目了然，**連接線也因此不需要**（線變成長對角線、只有雜訊沒有資訊） |
| **active = 藍底黑字實心藥丸** | 原本是「深色藥丸 + 後面墊藍板」，藍色只從邊緣透出來，讀起來像反白條而不是選中的牌子（使用者回報）。改成直接把藥丸畫成藍色 |
| **hover 顯示 pointer 游標** | 3D 層第一次做 cursor。**用引用計數**：20 塊牌子會重疊，移動時新的 `pointerover` 比舊的 `pointerout` 先觸發，單純設/清會被後到的 leave 擦掉 |
| **不處理「高樓超出畫面」** | 10 層實測頂樓會出畫面。**使用者決定不做自動取景**——30/50 層時任何自動框景都救不了，硬做只會犧牲常見的 3~5 層案例。高樓用電梯條切或自己拉遠即可 |

**過程中修掉的兩個真 bug（都不是測試問題）**
1. **Label3D 的 sprite 會吃掉點擊**：sprite 預設可 raycast 且 `depthTest:false`，會擋在自己的 hit plane 前面把點擊吞掉。→ 加 `interactive={false}`（既有呼叫端預設 `true`，行為不變）。
2. **點牌子會讓升降停在半路**：OrbitControls 自己綁在 canvas 上的 pointerdown，**r3f 的 `stopPropagation` 擋不住**，於是 55-F 的 `yieldToUser` 把「點牌子」誤判成「要抓相機」，把這次點擊剛要求的升降取消掉。→ `tabPointerGuard`（400ms 時窗旗標），牌子在 pointerdown 標記手勢屬於 UI，`yieldToUser` 見到就放行。

**Label3D 的 texture cache key 必須含 variant**：原本只用 `text` 當 key，加了配色變體後若不改 key，同一個樓層名第一次算出什麼顏色、之後全域都會拿到那個顏色。

**驗證（0 console errors）**
- 5 層：10 塊牌子（5×2）、五層點擊 camY 全對（1F=25.89 / 2F=28.89 / 3F=31.89 / 4F=34.89 / 5F=37.89，每層差 3m）
- **繞一整圈 5 個停點，牌子數量恆為 10**——沒有任何出現/消失
- 游標：牌子上 `pointer`、離開 `(empty)`、牌子間移動不中斷
- 10 層壓測：牌子高 28.7px，近端列間距 82~151px 好讀、遠端列最擠 55px；頂樓超出畫面（已決定不處理）
- 截圖 `.playwright-mcp/p56-01` ~ `p56-16`

> **量測陷阱**：驗 3D 可點物件要用 **Raycaster 實際打一次**確認自己的 hit target 是第一個命中物，
> 光看座標/截圖會漏掉「有東西擋在前面」。另外相機升降期間投影座標會一直變，
> **點擊測試前要先等相機真的靜止**，否則會誤判成「點不到」。
...
> - [x] **G10 UI/UX 與匯出正確性 ✅**（2026-08-19，12 修 / 3 報告過期 / 5 使用者決定不改）
>
> **★ Phase 53 十組全部完成（2026-08-19）。93 條分類收斂為：實修 ~70 條、報告過期或誤判 11 條、
> 使用者決定不改 6 條、需決策未動 6 條（見 groups 檔末節）。**
>
...
>
> **★ T13 —— G1 當時的描述後來查證有誤，已在 G6 更正（2026-08-14）**：
> G1 時我報告「同一支相機關掉再開，第一次開是空的、第二次才載入 4 點」。
> G6 用 stash 反證重測（含「透過 modal 儲存後再開」的完整序列），
> **原始碼在所有序列下都正確顯示 4 點，無法重現** —— 當時應是 React state 殘留造成的誤讀。
> **但那個 effect 確實有兩個真實缺陷**（`if (!camera) return` 讓關閉時保留前一支相機的點；
> dep 只看 id），G6 改成 dep 為 `camera` 物件並移除該 early return。
>
...
>
> **G10 收工小結（2026-08-19，防重做）**：使用者選擇「一次做完」。修 12 條、**3 條報告過期**、
> **5 條使用者決定不改**。有反證的三條最重要：
> **P0-4**：對話框無 focus trap —— Tab 一下焦點就跑到 topbar（＋新增樓層），此時 Enter
> **刪掉 5 道牆**（45→40）：焦點顯示一件事、按鍵做另一件事。修法除了 trap，**預設焦點從
> 紅色「刪除」改到「取消」**（destructive 動作不該預先上膛）。⚠ 第一版把 Enter 交給瀏覽器
> 原生按鈕啟動 → 兩個按鈕都失效，驗證才抓到，改成明確路由到 focused 按鈕。
> **P0-3**：`handleApply` 讀當下 `activeFloorId`，但預覽 overlay 刻意 click-through 讓使用者
> 能切樓層 → B 層規劃的 7 顆全寫進 A（5→12）。修法照 G6 慣例讓 `result` 自帶 `floorId`。
> **P0-6 / E6 同根因**：`floor.elevation` **這個欄位根本不存在**（全庫無 setter）。P0-6 讓
> riser 垂直 dz 恆為 0（跨樓層免費，Dijkstra 可能繞路）；E6 讓 RiserPanel 排序比 0 vs 0
> 且高度標籤**永不顯示**。兩處都改用 `computeFloorElevations`（3D 用的同一個來源）。
>
> **23l 的修法刻意放大範圍**：八個用 `useOverlayDismiss` 的 overlay 有**六個完全沒有 Esc**。
> 逐一補只會再漏下一個，所以 Escape 併進 hook 本身；`disabled` 的 caller 傳 null 就不掛監聽，
> 「執行中不可關閉」語意保留。ConfirmDialog / ScaleDialog 自己也處理 Esc，重複呼叫同一個
> onDismiss 是幂等的。
>
> **P1-11 的坑（兩個 guard 打架）**：`TextInput` 每個鍵都直寫 store，清空即提交空名。加 Esc
> 還原後**第一版還原成 AP-06** —— 因為 52-D9 的 blur guard 看到還是空值就補了新名字。修法是
> **blur 延後一個 tick**，讓還原先提交。四路徑全驗：清空+Esc→原名、改字+Esc→原名、
> 真改名+Tab→提交、清空+Tab→走 52-D9 fallback（不是空字串）。
>
> **P2-30 使用者兩次介入才定案**：① 先做成一顆會變字的按鈕 → 使用者指出「看不出可以點」
> → 改成**兩顆並排、active 高亮**的分段控制（照 TrendPanel `__seg` 慣例，不自創元件）。
> ② 切換鈕原本放標題旁，**剛好佔住表頭正中央**搶走「點表頭展開」的點擊（stash 反證確認是
> 自己造成的），移到數字後面。③ 本層模式下 BOM 明細仍是全案數字 —— **依設計如此**（採購用，
> per-floor 另有「每樓纜線」區塊），加一行說明而非讓兩種範圍無聲並存。
>
> **P2-31 是誤報 + 真問題各半**：「要點兩下」誤報（實測一下開選單、一下選工具，hover 還會
> 自動展開）。真問題是 **30×30px 純圖示零文字**，7 個群組都得 hover 才知道是什麼。但加文字
> 或放大按鈕都會讓工具列變寬，直接撞到已決定不改的 P2-25，故一併不改。
>
> **E8 的 PNG 匯出最值得記**：原本 `setTimeout(doExport, 50)` 是猜的 —— repaint 沒趕上時
> PNG 是**上一層的內容配新樓層的檔名**（一份錯圖，名字卻很有說服力）。改成 220ms（對齊 PDF
> exporter 的 REPAINT_SETTLE_MS）+ 兩個 rAF + 驗證 store 真的切了。
>
> **順手發現、使用者決定不記的既有 bug**：`exportPlanView.js:34-35` 匯出時把 `world.scale`
> 設為 1 但不重畫 gStatic，PNG 線寬因此取決於按下匯出當時的 zoom —— 同一份規劃縮小匯出
> 線材佔 5799 px、放大只佔 2572 px（**差 2.25 倍**）。
>
