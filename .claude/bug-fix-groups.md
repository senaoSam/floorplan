...
**★ T13(相機校正重開)—— G1 當時的描述後來查證有誤,已於 G6 修正並更正紀錄**:
G1 時我報告「同一支相機關掉再開,第一次開是空的、第二次才載入 4 點」。
G6 用 stash 反證重測(含「透過 modal 儲存後再開」的完整序列),**原始碼在所有序列下都正確顯示 4 點,無法重現**。
當時的觀察應是 React state 殘留造成的誤讀,不是 effect 沒重跑。
**但那個 effect 確實有兩個真實缺陷**,G6 一併改掉(見 G6 專節 T13 列)。
...
結果:**只有一個真實分歧** —— 同樓板兩個洞 61.24 dB(= P1-7,G3 已修)。
另外兩個「反射 10.07 dB / 反射+繞射 6.97 dB」**後來查證是 harness 傳錯 `itu` 型別造成的假分歧**,
真實材質下引擎本來就一致(見下方 R1 結案)。完整表格與三個比對陷阱見下方「parity 全面掃描」。
...
**沒有剩下的分歧**:曾以為反射路徑另有一條(~10 dB),後來查證是 harness 傳錯 `itu` 型別 —— 見 R1 結案。
...
| ~~反射 `maxReflOrder:1`~~ | ~~10.07 dB / 197 格~~ | ❌ **作廢 —— 是 harness 傳了 `itu: 0`,見下方 R1 結案** |
| ~~反射 + 繞射~~ | ~~6.97 dB / 153 格~~ | ❌ 同上;真實材質下為 0.000 |
...
2. ~~**反射路徑是新發現的分歧**(~10 dB)~~ **❌ 誤判,已作廢** —— harness 傳了 `itu: 0` 讓兩個引擎算不同材質(見下方 R1 結案)。**引擎在反射路徑上本來就一致。**
...
# G4 · GL 例外路徑與資源回滾 ✅ **完成**(2026-08-14,使用者驗收 ok;R1 查證為誤判、非 bug)
...
| ~~R1~~ ❌ | ~~反射路徑,`maxReflOrder ≥ 1`~~ | **不是 bug。是我的測試 harness 傳了 `itu: 0`**(`itu` 是物件不是數字)讓 JS 當金屬、GL 當退化材質 → 假分歧。真實 ITU 材質下原始碼即為 **0.000 dB**。詳見下方 R1 結案段落 | 結案,無需修改 |
...
**R1 已結案**:查證為測試 harness 的錯,不是 bug —— 見下方獨立段落。**G4 至此全部完成。**

---

## R1 · 反射路徑「分歧」—— ❌ **不存在。是我的測試 harness 寫錯,不是 bug**(2026-08-14 結案)

**根本原因:我的測試 scenario 傳了 `itu: 0`,而 `itu` 是物件 `{a,b,c,d}` 不是數字。**
- JS `materialEpsC:137` 是 `if (!itu || itu.metal) return { metal: true }` → `0` 是 falsy → **JS 當成金屬**(Γ = −1)
- GL `materialEpsC` 讀 `itu.x = 0` → 算出退化的 `epsC` → **GL 當成某種怪材質**
- **兩邊算的是不同材質**,難怪差 10 dB。真實材質的 itu 見 `constants/materials.js`(例:混凝土 `{a:5.24, b:0, c:0.0462, d:0.7822}`)

**用真實 ITU 材質重測,原始碼(未做任何修改)結果**:

| 情境 | dead cells | parity |
|---|---|---|
| 混凝土,AP 在同列(原「必死」情境) | **0** | **0.000 dB** |
| 輕隔間,AP 在同列 | **0** | **0.000 dB** |
| `{metal:true}`,AP 在同列 | **0** | 0.001 dB(fp32) |
| 混凝土,AP 離軸 0.5m | **0** | **0.000 dB** |
| 兩道牆 + 反射(原「10.07 dB」情境) | — | **0.000 dB** |
| 反射 + 繞射(原「6.97 dB」情境) | — | 1.569 dB / **1 格** |

最後那 1 格已查證是**破壞性干涉深谷的 fp32 相位噪音**:該格 GL −60.33 / JS −61.90,而左鄰 −47.06、右鄰 −76.19 **完全一致**。`propagationGL.js:930` 的註解本來就記載這類 outlier(已用 phase-referencing 壓到遠低於原本的 ~30 dB)。**不是公式分歧。**

**因此前面掃描表裡「反射 10.07 dB / 反射+繞射 6.97 dB」兩列作廢**,`Hpara` NaN 的整套推論也隨之作廢(那是餵了退化 `epsC` 之後的下游現象,不是獨立缺陷)。

**我嘗試過的修法已撤回**:把 `addPathHN` 的兩個 `inout vec2[NMAX]` 併成單一 `vec4[NMAX]`。用 stash 反證後確認**數字完全不變**(11 格死、10.074 dB 一模一樣),證明雙 `inout` 陣列不是原因,所以沒有保留該改動。

**教訓(寫給下一次的自己)**:
1. **建 scenario 時欄位型別要照真實資料**。`itu` 是物件、`material` 是物件 —— 傳數字或 `0` 會讓兩個引擎走完全不同的分支,製造出看起來很嚴重的假分歧。**優先用 `buildScenario` 產生 scenario**,而不是手寫。
2. 這是本次 Phase 53 第**三**次同類誤判(前兩次:sentinel −300 vs 巨大負值、`CULL_FLOOR_DBM` −120)。**parity harness 的假陽性比真 bug 還多** —— 看到大數字先懷疑 harness。
3. 反證(stash 掉修改看數字會不會變)是最快的判別法。這次它在我寫錯修法後立刻抓到。
...
# G6 · 切樓層 / 切模式的狀態殘留 ✅ **已完成**(2026-08-14,使用者驗收 ok)
...
**修法總覽**:把切樓層清理集中到**那一個既有的 `prevFid` 邊緣偵測 subscriber**(而非散落各處加守衛),因為 `setActiveFloor` 刻意保持 dumb;另外三條改用**「payload 自帶 floorId + 讀者自己比對」**(`sessionWallIds`、gap marker、`prevHById`),即使沒人清理也不會作用在錯樓層。

| 條目 | 修法 | 驗證 |
|---|---|---|
| T6 | `clearDraft()` 補清 `doorWindowDraft` | ✅ |
| P4#1 | 集中處理器呼叫 `clearDraft()` | ✅ |
| T5/P4#4 | `cam.setDrawTool(null)`(順帶清 `draftPoint`) | ✅ |
| P1-13/P4#6 | `cam.closeLiveView()` | ✅ |
| P4#7 | `cam.closeCalibrate()` | ✅ |
| P4#5 | 呼叫既有但**從未被呼叫過**的 `useClientViewStore.reset()` | ✅ |
| P1-24 | `clearSelected()` + `setReading(null)`(兩個獨立症狀) | ✅ |
| 23t/P4#8 | `flashGapMarker` payload 加 `floorId`,layer 加守衛 | ✅ |
| **23n** | `getScenario` 回 null 或 0 AP 時,連 `associationArea`/`singleApArea` 一起清 | ✅ **反證+截圖** |
| T13 | `CalibrationModal` 的載入 effect:dep 由 `[calibrateCameraId]` 改為 `[camera]`,並移除 `if (!camera) return`(關閉時保留前一支相機的點) | ✅ 三次開關都顯示 4 點 |
| P4#2 | `sessionWallIds` 加 `sessionFloorId`,跨樓層時重置 | ⚠️ 見下 |
| P4#3 | `prevHById` 加 `prevHFloorId` 邊緣偵測 | ⚠️ 見下 |
| P4#9 | **不需修** —— `useAutoPlaceStore` 已是自帶 floorId 的正確寫法 | — |

## 驗收結果(兩輪獨立,全程 0 console error)

**輪 1**:store 層逐項驗證 8 條,全部 before/after 明確(如 draft pts 3→0、`drawTool` `'tripwire'`→null、`selectedId`→null)。
**輪 2**:改用**真實 sidebar 點擊**切樓層(不是 `setActiveFloor`),6 項 transient state 一次全清。

**★ 23n 是本組唯一有完整反證的真 bug**:
| | 修前(stash 反證) | 修後 |
|---|---|---|
| 0 AP 後 `associationArea` | **仍在** | null |
| 0 AP 後 `singleApArea` | **仍在** | null |
| `reading` | 已清(所以面板看起來正常) | 已清 |

**截圖對照**:修前那張是 0 AP、面板讀數全空,但**粉紅+藍覆蓋層仍蓋滿整個平面圖** —— 正是報告描述的症狀(`.playwright-mcp/g6-23n-baseline-after.png`)。輪 2 換成「AP 一顆一顆刪」:4/3/2/1 時覆蓋層正確保留,**剛好在 0 時清除**。

## ⚠️ 兩條改了但**無法重現原報告的後果**(誠實記錄)

**P4#2(`sessionWallIds` 跨樓層)** —— 用真實滑鼠畫 3 段牆 → 切 2F → Backspace,試了 4 種設置(兩者都關 / 只關 P4#1 / 切去切回 / Backspace ×3),**1F 一律 0 損失**。同層 Backspace 確實有效(48→47),所以 step-back 機制本身是通的;但跨樓層時 `walls.find()` 在新樓層找不到舊 id,迴圈把清單排空後走「只剩 anchor → 丟掉 anchor」分支,**沒有刪到任何牆**。報告該條屬 P4 系列(靜態讀碼),可能是推論而非實測。

**P4#3(`prevHById` 跨樓層)** —— **找到了為什麼不可能發生**:`useTrackingStore.js:83` 是
```js
const frameSamples = trk.frameSamples ?? project(base, Hinv)
const samples = project(frameSamples, H)
```
先用 `H⁻¹` 再用 `H` 投影 —— **數學上恆等變換**。所以「用同一個 H 重複 reproject」不可能改變座標。實測 4 種設置(含 1F 相機也校正過、1816 條軌跡確實綁定相機)checksum 全部維持 `151050939.329` 不變。

**兩條的修改都保留**:語意更正確(狀態本來就只在自己樓層有意義)、零風險、且與同組其他條共用同一個 pattern。但**不能聲稱驗證了原症狀**。

**T13 也同屬這一類 —— 而且錯的是我自己的 G1 紀錄**。G1 時我寫「第一次開是空的、第二次才載入」,G6 用 stash 反證重測(含透過 modal 儲存後再開的完整序列),**原始碼一律正確顯示 4 點**。當時應是 React state 殘留的誤讀。effect 本身的兩個缺陷是真的(`if (!camera) return` 讓關閉時留著前一支相機的點;dep 只看 id),已改。

## 本組的方法論教訓

**93 條報告裡的 P4 系列是靜態讀碼推論,不是 QA 實測。** G6 這 12 條中,4 條(P4#2、P4#3、T13,加上 G2 的反射誤判)的**後果描述無法重現**。加上 Phase 53 前面三次 harness 假陽性,累計 **7 次「看起來很嚴重但驗不出來」**。

**下一組(G7~G10)的建議**:
1. **先寫能失敗的測試,再改**。stash 反證應該是**第一步**而不是最後一步 —— 我在 G6 是先改完 12 條才發現 3 條驗不出來。
2. 報告條目標「QA 實測」的優先做(那些後果是真看到的);標 P4/靜態分析的先花 10 分鐘確認能不能重現。
3. **報告的「後果」欄不可信,「位置」欄可信** —— 12 條的程式碼位置全部正確,錯的都是推論出來的使用者可見後果。
