...
| HM-F5b | ⚠️ 已知 bug | Uniform Grid 空間加速（不用 BVH，2D 場景下 grid 構造簡單、shader 無遞迴）：CPU 端按 wall AABB 把每面牆塞進覆蓋的 cells，emit `gridIdxTex` (RGBA32F, nGx×nGy) + `gridListTex` (R32F, flat wall idx)。Shader 用 Amanatides-Woo DDA 沿 ray 走 cells，配 8-slot 循環 buffer 去重。`setUseGrid(false)` 可切回 brute force（debug / bench 用）。**驗收**：對 basic friis baseline max diff = 0.000 dB ✓。Bench (`#/heatmap-bench`) 100 AP × 2000 牆 JS 155s vs Shader+grid 129ms → **1196× 加速**。**已知 bug（2026-04-25 發現）**：對 `__fixtures__/refl-min/`（1 垂直 metal wall）grid traversal 漏算 wall loss → max 44 dB diff vs friis baseline；切到 `setUseGrid(false)` brute force 通過。bug 只在「單一垂直牆 + ray 跨牆」極端情形觸發；basic fixture（多 walls）平均化掩蓋。修復方向：DDA 邊界 / cell-bbox 對齊 ／ seenBuf 去重邏輯三選一。修復前提：F5g 重組後可能 wall accumulation 邏輯整個重寫，bug 自動消失，因此**先觀察 F5g 進度再決定是否單獨修** |
| HM-F5b-fix | ⏸️ | 修復 HM-F5b 對「單一垂直牆」grid traversal 漏算 bug（refl-min fixture friis baseline 應該 ≤ 1 dB）。延後到 F5g 開工前再決定是否單獨處理（F5g 可能整個重寫 wall accumulation pipeline 讓此 bug 自動消失）|
