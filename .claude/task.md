...
| 31-3b | ⬜   | **Heatmap 整合（rest of 31-3）** — 既有 `heatmapGL.js` raw WebGL2 canvas 不動；新 `src/render/heatmapAdapter.js` 把 canvas 包成 `PIXI.Texture.from(canvas)` → PIXI.Sprite 掛 Layer 3；hover readout / colormap 行為保留。需先 port `useAPStore` / `useWallStore` / `useHeatmapStore` + heatmapGL.js 自身 | 0.5 天 |
...
| 31-6  | ⬜   | **Devices — AP markers** — PIXI.Sprite + texture atlas（or SDF atlas）；1000 AP batch 1 draw call；frequency color / direction / pattern 透過 sprite frame 或 tint；uniform grid spatial index for hit-test | 2 天 |
| 31-7  | ⬜   | **Devices — Switches + Risers** — PIXI.Container + Sprite（switches 含 chassis + port row + label + decoration；risers 單 sprite）；100 SW 走 PixiJS interaction OK | 1 天 |
| 31-8  | ⬜   | **Trays** — PIXI.Graphics for body + magnet（量少 50），vertex handle + segment hit 自寫 Container；body drag / vertex drag 沿用 Phase 24-30-2 dragOverlay pattern；snap halo 即時顯示 | 3 天 |
