...
| HM-T5  | 部分 | Edge-case fixtures：✅ `__fixtures__/refl-min/`（1 metal wall + 1 AP，反射隔離 debug 用）— 2026-04-25 為 F5c+d 嘗試而建。✅ `__fixtures__/dense-aps/`（100 AP 10×10 jittered + ~600 walls cubicle + metal islands；觸發 HM-F5h cascade gate apCount≥50；驗 aggregated path 在大規模下的數值正確性 + bench 平台）— 2026-04-26。✅ `__fixtures__/dense-walls/`（3 AP + ~50 walls 壓縮在 30×20 m corridor maze；refl=off 純 Friis+walls 路徑壓力測試）— 2026-04-26。**附帶修復**：原 SEEN_BUF=8 cyclic-dedup buffer 在 dense-walls 場景失效（5 dB RSSI / 14 dB CCI friis-baseline drift），bump 到 16 後全 PASS（max 0.093 dB ≤ 1 dB）；basic + refl-min + dense-aps 不 regress。⬜ `__fixtures__/cross-floor-tunneling/`（斜射穿多 slab 驗 sec(θ)）|
...
| HM-F5c-fix-2 | ❌ 嘗試失敗 | **Kahan/Neumaier H-accumulator compensated sum 試過完全無效**（2026-04-26 MCP 驗證 F5d gate）：basic outlier 仍然 max 32.06 dB / 1 cell，數字跟 fix-2 前一字不差（mean 0.019 / p95 0.000 / >1dB cells = 1）。**原因不是 fp32 round-off 累加**：(10, 13) m 這格 rx 精確坐在 metal wall (x=10) 軸線上 + AP-A 也在同軸 (10, 7.5) → reflection paths 跟 direct path **算術上**完全等幅反相，跟累加精度無關，Kahan 救不了。改動已 revert。**下個方向**：(a) sentinel + JS fallback for extreme nulls（host 端偵測 RSSI < -250 dBm 該 cell 用 JS 重算），或 (b) 接受 known-issue（1/2501 ≈ 0.04% 視覺面積，rx 落在 metal wall 軸線是 grid 對齊到 0.5 m 才會踩的偶發）。**已驗證確認**：影響範圍是 single cell 不擴散——周圍 5×5 = 25 cells 中 24 個 d_rssi = 0.00 |
